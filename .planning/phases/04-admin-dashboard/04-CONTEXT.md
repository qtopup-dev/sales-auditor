...
- **D-17:** Invite flow: "Invite Moderator" button in the UsersPage header calls `POST /api/auth/invite` (already implemented — route is `/invite`, confirmed in auth.ts line 132, NOT `/invite-generate`). Opens a modal displaying the generated invite link with a "Copy Link" button. Admin shares the link manually. No new backend endpoint needed for invite — existing endpoint is reused.
